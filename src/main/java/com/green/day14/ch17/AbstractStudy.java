...
public abstract class AbstractStudy {
    ...
    class 앞에 abstract를 붙이는 것은 선택사항인 ㅏ만약에 추상 메소드를 가지게 된다면
    선택사항이 아니라 필수사항이 된다.

    클래스간의 공톡적인 부분을 부모에 작성을 헤서 상속받고록 하기 위한 것.
...
abstract class ImplementsStudy2 extends AbstractStudy{ //추상메소드를 구현,오버라이딩 하지 않으면 빨간줄이 나온다
